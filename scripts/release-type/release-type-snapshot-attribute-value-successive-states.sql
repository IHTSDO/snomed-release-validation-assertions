...
********************************************************************************/
	...
	select 
		...
		concat('AttributeRefsetId=',d.id, ' is inactive but no active state found in the previous snapshot.'),
		d.id,
		'curr_attributevaluerefset_s'
	from ( select a.id, a.referencedcomponentid from curr_attributevaluerefset_s a left join prev_attributevaluerefset_s b
	on a.id = b.id
	where a.active = 0
	and b.id is null
	and not exists (select 1 from curr_attributevaluerefset_f c where a.id = c.id and c.active = 1 and cast(c.effectivetime as datetime) < cast(a.effectivetime as datetime))) d 
	left join dependency_attributevaluerefset_s e on d.id = e.id
	where e.id is null;
