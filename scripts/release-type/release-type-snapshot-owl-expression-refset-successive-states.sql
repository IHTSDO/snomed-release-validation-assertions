...
********************************************************************************/
	...
	select 
		...
		concat('Owl Expression: id=',d.id, ': is inactive but no active state found in the previous release.'),
		d.id,
		'curr_owlexpressionrefset_s'
	from (select a.id, a.referencedcomponentid from curr_owlexpressionrefset_s a left join prev_owlexpressionrefset_s b
	on a.id = b.id
