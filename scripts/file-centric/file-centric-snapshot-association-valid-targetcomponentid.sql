...
********************************************************************************/
	...
	create table if not exists v_concept_and_description_ids as
	select id from curr_concept_s  
	union all
	select id from curr_description_s;

	alter table v_concept_and_description_ids add index idx_concept_and_description_id(id);
	...
	left join v_concept_and_description_ids b
	on a.targetcomponentid = b.id
	where b.id is null;
	commit;

	drop table if exists v_concept_and_description_ids;
