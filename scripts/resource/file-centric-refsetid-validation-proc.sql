...
set @sql = CONCAT('insert into qa_result(run_id, assertion_id, concept_id, details, component_id) select ', runid,',',assertionid,',0,',@details, ',a.id from (select min(id) as id, refsetid from ', substring_index(dbname,'.',1),'.',tb_name, ' where refsetid not in (select refsetid from ',substring_index(dbname,'.',1),'.refset_id) group by refsetid) a;');
