...
********************************************************************************/
drop procedure if exists validate_changes_in_expected_modules_edition;
create procedure validate_changes_in_expected_modules_edition(dbname char(255),runid BIGINT, assertionid char(36))
begin
...
myloop: loop fetch table_cursor into tb_name;
	...
	if (tb_name != "moduledependencyrefset_d") then
		...
		set @sql = CONCAT('insert into qa_result(run_id, assertion_id,concept_id, details) select ', runid,',',assertionid,',0,',@details,' from (select id,moduleid from ', substring_index(dbname,'.',1),'.',tb_name, '  where moduleid not in (select distinct referencedComponentId from ',substring_index(dbname,'.',1),'.moduledependencyrefset_d where active="1" UNION select distinct moduleid from ',substring_index(dbname,'.',1),'.moduledependencyrefset_d where active="1")) a;');
